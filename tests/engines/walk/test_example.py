...
@pytest.fixture(scope="module")
def skeleton():
    return json.loads((WALK_EXAMPLE / "skeleton.json").read_text(encoding="utf-8"))
...
@pytest.fixture(scope="module")
def crops():
    return json.loads((WALK_EXAMPLE / "crops.json").read_text(encoding="utf-8"))
...
def test_the_recorded_crops_assemble_a_bundle_that_validates(paper, crops,
    ...
    assert load_script("tools/validate_bundle.py").main([str(bundle)]) == 0
